...
def Game(numPlayers):
    # Game will call Turn() and Turn() will implement a while loop to avoid recursion
    scores = [0] * numPlayers
    ...
    print()
    DisplayTurnAndScores(numPlayers, scores, currentPlayer, True)
...
def DisplayTurnAndScores(numPlayers, scores, currentPlayer, end = False):
    widthOfScreen = 90
    sizeOfText = 14
...
def ParseNumbers(numbers):
    # A set will ignore multiple selections of the same die
    numset = {}
    for num in numbers:
        if IsInvalid(num):
            print("Invalid input")
